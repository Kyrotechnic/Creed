...
public class FreeCam extends Module
{
    ...
    public void onEnable() {
        if (Kore.mc.theWorld != null) {
            (this.playerEntity = new EntityOtherPlayerMP(Kore.mc.theWorld, Kore.mc.thePlayer.getGameProfile())).copyLocationAndAnglesFrom(Kore.mc.thePlayer);
            this.playerEntity.onGround = Kore.mc.thePlayer.onGround;
            Kore.mc.theWorld.addEntityToWorld(-2137,this.playerEntity);
        }
    }
    ...
    public void onRenderWorld(final RenderWorldLastEvent event) {
        if (this.isToggled() && this.playerEntity != null && this.tracer.isEnabled()) {
            RenderUtils.tracerLine(this.playerEntity, event.partialTicks, 1.0f, Kore.themeManager.getSecondaryColor());
        }
    }
