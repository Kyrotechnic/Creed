package baby.creed.util.render;

import baby.creed.Creed;
import baby.creed.util.WorldToScreen;
...
//        GL11.glHint(3155, 4352);
    ...
    public static void scissor(double x, double y, double width, double height) {
        final ScaledResolution sr = new ScaledResolution(Creed.mc);
    ...
    public static void draw2D(final Entity entityLiving, final float partialTicks, final float lineWidth, final Color color) {
        ...
        GL11.glPushMatrix();
        GL11.glLoadIdentity();
        GL11.glOrtho(0.0, (double)Creed.mc.displayWidth, (double)Creed.mc.displayHeight, 0.0, -1.0, 1.0);
        ...
        final RenderManager renderManager = Creed.mc.getRenderManager();
        ...
        for (final double[] boxVertex : boxVertices) {
            final Vector2f screenPos = WorldToScreen.worldToScreen(new Vector3f((float)boxVertex[0], (float)boxVertex[1], (float)boxVertex[2]), mvMatrix, projectionMatrix, Creed.mc.displayWidth, Creed.mc.displayHeight);
            if (screenPos != null) {
        ...
        if (minX > 0.0f || minY > 0.0f || maxX <= Creed.mc.displayWidth || maxY <= Creed.mc.displayWidth) {
            GL11.glColor4f(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, 1.0f);
            GL11.glBegin(2);
    ...
    public static void drawTargetESP(final EntityLivingBase target, final Color color, final float partialTicks) {
        ...
        GlStateManager.translate(target.lastTickPosX + (target.posX - target.lastTickPosX) * partialTicks - Creed.mc.getRenderManager().viewerPosX, target.lastTickPosY + (target.posY - target.lastTickPosY) * partialTicks - Creed.mc.getRenderManager().viewerPosY + target.height * location, target.lastTickPosZ + (target.posZ - target.lastTickPosZ) * partialTicks - Creed.mc.getRenderManager().viewerPosZ);
        enableGL2D();
    ...
    public static void drawImage(final ResourceLocation image, final float x, final float y, final float width, final float height, final float alpha) {
        ...
        Creed.mc.getTextureManager().bindTexture(image);
    ...
    public static void blockBox(final TileEntity block, final Color color) {
        ...
        GL11.glDepthMask(false);
        setColor(color);
        RenderGlobal.drawSelectionBoundingBox(block.getRenderBoundingBox().offset(-Creed.mc.getRenderManager().viewerPosX, -Creed.mc.getRenderManager().viewerPosY, -Creed.mc.getRenderManager().viewerPosZ));
        GL11.glEnable(3553);
        GL11.glEnable(2929);
        GL11.glDepthMask(true);
        GL11.glDisable(3042);
    ...
    public static void renderStarredNametag(final Entity entityIn, final String str, final double x, final double y, final double z, final int maxDistance) {
        final double d0 = entityIn.getDistanceSqToEntity(Creed.mc.getRenderManager().livingPlayer);
        if (d0 <= maxDistance * maxDistance) {
            final FontRenderer fontrenderer = Creed.mc.getRenderManager().getFontRenderer();
            final float f = 1.6f;
            final float f2 = 0.016666668f * f;
            GlStateManager.pushMatrix();
            GlStateManager.translate((float)x + 0.0f, (float)y + entityIn.height + 0.5f, (float)z);
            GL11.glNormal3f(0.0f, 1.0f, 0.0f);
            GlStateManager.rotate(-Creed.mc.getRenderManager().playerViewY, 0.0f, 1.0f, 0.0f);
            GlStateManager.rotate(Creed.mc.getRenderManager().playerViewX, 1.0f, 0.0f, 0.0f);
    ...
    public static void renderLivingLabel(final Entity entityIn, final String str, final double x, final double y, final double z, final int maxDistance) {
        final double d0 = entityIn.getDistanceSqToEntity(Creed.mc.thePlayer);
        if (d0 <= maxDistance * maxDistance) {
            final FontRenderer fontrenderer = Creed.mc.getRenderManager().getFontRenderer();
            final float f = 1.6f;
            final float f2 = 0.016666668f * f;
