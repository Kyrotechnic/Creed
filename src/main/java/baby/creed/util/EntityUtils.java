...
public class EntityUtils
{
    private static boolean isOnTeam(final EntityPlayer player) {
        for (final Score score : Creed.mc.thePlayer.getWorldScoreboard().getScores()) {
    ...
    public static boolean isTeam(final EntityLivingBase e2) {
        ...
        return Creed.mc.thePlayer.getDisplayName().getFormattedText().charAt(2) == Creed.fancy && e2.getDisplayName().getFormattedText().charAt(2) == '?' && Creed.mc.thePlayer.getDisplayName().getFormattedText().charAt(3) == e2.getDisplayName().getFormattedText().charAt(3);
    }
