...
import baby.creed.events.*;
import baby.creed.util.PlayerUtils;
import baby.creed.Creed;
import baby.creed.managers.CommandManager;
import baby.creed.modules.combat.Aura;
import baby.creed.util.MovementUtils;
import baby.creed.util.RotationUtils;
...
public abstract class PlayerSPMixin extends AbstractClientPlayerMixin
{
    ...
    public void jump() {
        ...
        if (this.isSprinting() && MovementUtils.isMoving()) {
            final float f = ((Creed.sprint.isToggled() && Creed.sprint.omni.isEnabled()) ? MovementUtils.getYaw() : ((Creed.aura.isToggled() && Aura.target != null && Creed.aura.movementFix.isEnabled()) ? RotationUtils.getRotations(Aura.target).getYaw() : this.rotationYaw)) * 0.017453292f;
    ...
    public boolean isUsingItem(final EntityPlayerSP instance) {
        return !Creed.noSlow.isToggled() && (instance.isUsingItem() || Creed.autoBlock.isBlocking());
    }
    ...
    public void onLivingUpdate(final CallbackInfo ci) {
        if (Creed.sprint.omni.isEnabled() && Creed.sprint.isToggled()) {
        ...
        if (Creed.noSlow.isToggled() && (this.isUsingItem() || Creed.autoBlock.isBlocking())) {
            ...
            if (action == EnumAction.BLOCK) {
                ...
                movementInput.moveForward *= (float)Creed.noSlow.swordSlowdown.getValue();
                final MovementInput movementInput2 = this.movementInput;
                movementInput2.moveStrafe *= (float)Creed.noSlow.swordSlowdown.getValue();
            }
            else if (action == EnumAction.BOW) {
                final MovementInput movementInput3 = this.movementInput;
                movementInput3.moveForward *= (float)Creed.noSlow.bowSlowdown.getValue();
                final MovementInput movementInput4 = this.movementInput;
                movementInput4.moveStrafe *= (float)Creed.noSlow.bowSlowdown.getValue();
            }
            else if (action != EnumAction.NONE) {
                final MovementInput movementInput5 = this.movementInput;
                movementInput5.moveForward *= (float)Creed.noSlow.eatingSlowdown.getValue();
                final MovementInput movementInput6 = this.movementInput;
                movementInput6.moveStrafe *= (float)Creed.noSlow.eatingSlowdown.getValue();
            }
        }
        if (Creed.freeCam.isToggled()) {
            this.noClip = true;
        }
    }
    ...
    public void onUpdateAction(final CallbackInfo ci) {
        if (Creed.speed.isToggled() && !Speed.isDisabled() && MovementUtils.isMoving()) {
            ...
            if (f > 0.0f || f2 > 0.0f) {
                ...
                if (flag3) {
                    if (i > 0) {
                        ...
                        if (!Creed.sprint.isToggled() || !Creed.sprint.keep.isEnabled()) {
                            this.motionX *= 0.6;
                            this.motionZ *= 0.6;
                            this.setSprinting(false);
                        }
