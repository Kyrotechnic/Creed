...
public abstract class MixinItemRenderer {
    ...
    public void renderItemInFirstPerson(final float partialTicks) {
        ...
        if (this.itemToRender != null) {
            final boolean shouldSpoofBlocking = (Aura.target != null && !Creed.aura.blockMode.getSelected().equals("None")) || (!Creed.autoBlock.blockTimer.hasTimePassed((long)Creed.autoBlock.blockTime.getValue()) && Creed.autoBlock.canBlock());
            if (this.itemToRender.getItem() instanceof ItemMap) {
                ...
                switch (enumaction) {
                    ...
                    case BLOCK: {
                        if (Creed.animations.isToggled()) {
                            final String selected = Creed.animations.mode.getSelected();
                            switch (selected) {
                                case "1.7": {
                                ...
                                case "spin":
                                case "vertical spin": {
                                    this.transformFirstPersonItem(f, Creed.animations.showSwing.isEnabled() ? f2 : 0.0f);
                                    this.doBlockTransformations();
                                    break;
                                }
                                case "long hit": {
    ...
    private void transformFirstPersonItem(final float equipProgress, final float swingProgress) {
        final float size = (float) Creed.animations.size.getValue();
        final float x = (float)Creed.animations.x.getValue();
        final float y = (float)Creed.animations.y.getValue();
        final float z = (float)Creed.animations.z.getValue();
