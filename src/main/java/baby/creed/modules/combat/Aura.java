...
public class Aura extends Module {
    ...
    public void onTick(final TickEvent.ClientTickEvent event) {
        if (Creed.mc.thePlayer == null || Creed.mc.theWorld == null || !this.middleClick.isEnabled()) {
            return;
        }
        if (Mouse.isButtonDown(2) && Creed.mc.currentScreen == null) {
            if (Creed.mc.pointedEntity != null && !this.wasDown && !(Creed.mc.pointedEntity instanceof EntityArmorStand) && Creed.mc.pointedEntity instanceof EntityLivingBase) {
                final String name = ChatFormatting.stripFormatting(Creed.mc.pointedEntity.getName());
                if (!Aura.names.contains(name)) {
                    Aura.names.add(name);
                    Creed.notificationManager.showNotification("Added " + ChatFormatting.AQUA + name + ChatFormatting.RESET + " to name sorting", 2000, Notification.NotificationType.INFO);
                }
                else {
                    Aura.names.remove(name);
                    Creed.notificationManager.showNotification("Removed " + ChatFormatting.AQUA + name + ChatFormatting.RESET + " from name sorting", 2000, Notification.NotificationType.INFO);
                }
            }
            this.wasDown = true;
        }
    ...
    public void onMovePre(final MotionUpdateEvent.Pre event) {
        ...
        if (this.attackOnly.isEnabled() && !Creed.mc.gameSettings.keyBindAttack.isKeyDown()) {
            return;
        }
        if (Aura.target != null) {
            ...
            if (this.shovelSwap.isEnabled() && Aura.target instanceof EntityPlayer && this.hasDiamondArmor((EntityPlayer)Aura.target)) {
                this.lastSlot = Creed.mc.thePlayer.inventory.currentItem;
                for (int i = 0; i < 9; ++i) {
                    if (Creed.mc.thePlayer.inventory.getStackInSlot(i) != null && Creed.mc.thePlayer.inventory.getStackInSlot(i).getItem() instanceof ItemSpade) {
        ...
        if (Aura.target != null && Creed.mc.thePlayer.getDistanceToEntity((Entity)Aura.target) < Math.max(this.rotationRange.getValue(), this.range.getValue()) && this.attacks > 0) {
            ...
            while (this.attacks > 0) {
                ...
                if (Creed.mc.thePlayer.getDistanceToEntity((Entity)Aura.target) < this.range.getValue() && (RotationUtils.getRotationDifference(RotationUtils.getRotations(Aura.target), RotationUtils.getLastReportedRotation()) < 15.0 || this.rotationMode.is("None"))) {
                    Creed.mc.playerController.attackEntity((EntityPlayer)Creed.mc.thePlayer, (Entity)Aura.target);
            ...
            if (Creed.mc.thePlayer.getHeldItem() != null && Creed.mc.thePlayer.getHeldItem().getItem() instanceof ItemSword) {
                ...
                switch (selected2) {
                    ...
                    case "Hypixel": {
                        if (this.blockDelay.hasTimePassed(250L)) {
                            this.startBlocking();
                            Creed.mc.thePlayer.sendQueue.addToSendQueue(new C0BPacketEntityAction((Entity)Creed.mc.thePlayer, C0BPacketEntityAction.Action.STOP_SPRINTING));
                            Creed.mc.thePlayer.sendQueue.addToSendQueue(new C0BPacketEntityAction((Entity)Creed.mc.thePlayer, C0BPacketEntityAction.Action.START_SPRINTING));
    ...
    public void onRender(final RenderWorldLastEvent event) {
        if (this.isToggled() && Aura.target != null && this.lastAttack.hasTimePassed(1000 / this.nextCps) && Creed.mc.thePlayer.getDistanceToEntity((Entity)Aura.target) < (this.rotationSwing.isEnabled() ? this.getRotationRange() : this.range.getValue())) {
    ...
    private EntityLivingBase getTarget() {
        if ((Creed.mc.currentScreen instanceof GuiContainer && this.toggleInGui.isEnabled()) || Creed.mc.theWorld == null) {
            return null;
        }
        final List<Entity> validTargets = (List<Entity>)Creed.mc.theWorld.getLoadedEntityList().stream().filter(entity -> entity instanceof EntityLivingBase).filter(entity -> this.isValid((EntityLivingBase) entity)).sorted(Comparator.comparingDouble(e -> e.getDistanceToEntity((Entity)Creed.mc.thePlayer))).collect(Collectors.toList());
    ...
    private boolean isValid(final EntityLivingBase entity) {
        if (entity == Creed.mc.thePlayer || !AntiBot.isValidEntity((Entity)entity) || (!this.invisibles.isEnabled() && entity.isInvisible()) || entity instanceof EntityArmorStand || (!Creed.mc.thePlayer.canEntityBeSeen((Entity)entity) && !this.walls.isEnabled()) || entity.getHealth() <= 0.0f || entity.getDistanceToEntity((Entity)Creed.mc.thePlayer) > ((Aura.target != null && Aura.target != entity) ? this.range.getValue() : Math.max(this.rotationRange.getValue(), this.range.getValue())) || RotationUtils.getRotationDifference(RotationUtils.getRotations(entity), RotationUtils.getPlayerRotation()) > this.fov.getValue()) {
            return false;
        }
    ...
    private void startBlocking() {
        PacketUtils.sendPacket(new C08PacketPlayerBlockPlacement(Creed.mc.thePlayer.getHeldItem()));
