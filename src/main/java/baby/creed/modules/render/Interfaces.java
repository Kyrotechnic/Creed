...
import baby.creed.events.ScoreboardRenderEvent;
...
public class Interfaces extends Module
{
    ...
    private void renderScoreboard(final ScoreObjective p_180475_1_, final ScaledResolution p_180475_2_, final boolean customFont) {
        ...
        final int fontHeight = customFont ? (Fonts.getPrimary().getHeight() + 2) : Creed.mc.fontRendererObj.FONT_HEIGHT;
        ...
        final float arrayHeight = Creed.clickGui.getHeight();
        float j1 = p_180475_2_.getScaledHeight() / 2.0f + i1 / 3.0f;
        if (Creed.clickGui.arrayList.isEnabled()) {
        ...
        for (final Score score2 : collection) {
            ...
            if (s2.contains(Creed.fancy + "ewww.hypixel.ne\ud83c\udf82" + Creed.fancy + "et") && Creed.clickGui.waterMark.isEnabled()) {
                s2 = s2.replaceAll(Creed.fancy + "ewww.hypixel.ne\ud83c\udf82" + Creed.fancy + "et", "Creed Client");
            }
            ...
            final boolean flag = s2.equals("Creed Client");
            if (flag) {
                if (customFont) {
                    Fonts.getPrimary().drawSmoothCenteredStringWithShadow(s2, l1 + width / 2.0f, k2, Creed.themeManager.getSecondaryColor().getRGB());
                }
                else {
                    Creed.mc.fontRendererObj.drawString(s2, (int)(l1 + width / 2.0f - Creed.mc.fontRendererObj.getStringWidth(s2) / 2), (int)k2, Creed.themeManager.getSecondaryColor().getRGB());
                }
            }
            else {
    ...
    private void drawBorderedRoundedRect(final float x, final float y, final float width, final float height, final float radius, final float linewidth) {
        ...
        RenderUtils.drawGradientOutlinedRoundedRect(x, y, width, height, radius, linewidth, Creed.themeManager.getSecondaryColor(0).getRGB(), Creed.themeManager.getSecondaryColor(3).getRGB(), Creed.themeManager.getSecondaryColor(6).getRGB(), Creed.themeManager.getSecondaryColor(9).getRGB());
    }

    private void drawString(String s, final float x, final float y, final int color, final boolean customFont) {
        if (Creed.nickHider.isToggled() && s.contains(Creed.mc.getSession().getUsername())) {
            s = s.replaceAll(Creed.mc.getSession().getUsername(), Creed.nickHider.nick.getValue());
        }
        if (customFont) {
            Fonts.getPrimary().drawSmoothStringWithShadow(s, x, y, Color.white.getRGB());
        }
        else {
            Creed.mc.fontRendererObj.drawString(s, (int)x, (int)y, color);
        }
    }

    private float getStringWidth(final String s, final boolean customFont) {
        return customFont ? ((float)Fonts.getPrimary().getStringWidth(s)) : ((float)Creed.mc.fontRendererObj.getStringWidth(s));
    }

    @Override
    public void assign()
    {
        Creed.interfaces = this;
    }
}
