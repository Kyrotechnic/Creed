...
public class Gui extends Module {
    ...
    public Gui() {
        ...
        this.colorMode = new ModeSetting("Mode", "Astolfo", new String[] { "Rainbow", "Color shift", "Astolfo", "Vape", "Mint", "Devil"})
        {
            @Override
            public void cycle(int key)
            {
                ...
                Creed.themeManager.setTheme(colorMode.getSelected());
            }
        };
    ...
    public float getHeight() {
        ...
        final List<Module> list = Creed.moduleManager.getModules().stream().filter(module ->  (module.isToggled() || module.toggledTime.getTimePassed() <= 250L)).sorted(Comparator.comparingDouble(module -> Fonts.getPrimary().getStringWidth(module.getName()))).collect(Collectors.toList());
        float y = 3.0f;
    ...
    public void onRender(final RenderGameOverlayEvent.Post event) {
        if (event.type == RenderGameOverlayEvent.ElementType.CROSSHAIRS) {
            if (this.waterMark.isEnabled()) {
                Fonts.getSecondary().drawSmoothString("reed", Fonts.getSecondary().drawSmoothString("C", 5.0, 5.0f, Color.white.darker().getRGB()) + 1.0f, 5.0f, getColor().getRGB());
            }
            if (this.arrayList.isEnabled()) {
                GL11.glPushMatrix();
                final ScaledResolution resolution = new ScaledResolution(Creed.mc);
                final List<Module> list = Creed.moduleManager.getModules().stream().filter(module -> (module.isToggled() || module.toggledTime.getTimePassed() <= 250L)).sorted(Comparator.comparingDouble(module -> Fonts.getPrimary().getStringWidth(module.getName() + " | " + module.suffix()))).collect(Collectors.toList());
                ...
                for (final Module module2 : list) {
                    ...
                    Fonts.getPrimary().drawSmoothCenteredString(moduleName, resolution.getScaledWidth() - 1 - width / 2.0f, y + height / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, Creed.themeManager.getSecondaryColor(x).getRGB(), Creed.themeManager.getSecondaryColor(x - 1).getRGB());
                ...
                if (this.arrayOutline.isEnabled()) {
                    ...
                    for (final Module module3 : list) {
                        ...
                        addVertex((float)(resolution.getScaledWidth() - 1), y, (float)resolution.getScaledWidth(), y + height, Creed.themeManager.getSecondaryColor(x - 1).getRGB(), Creed.themeManager.getSecondaryColor(x).getRGB());
                        y += height;
                    }
                    tessellator.draw();
    ...
    public void onEnable()
    {
        ...
        Creed.mc.displayGuiScreen(modernClickGui);
    }

    @Override
    public void onDisable()
    {
        Creed.mc.displayGuiScreen(null);
    }

    public Color getColor() {
        return Creed.themeManager.getSecondaryColor(0);
    }
    @Override
    public void assign()
    {
