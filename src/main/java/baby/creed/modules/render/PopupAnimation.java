...
public class PopupAnimation extends Module
{
    ...
    public static boolean shouldScale(final GuiScreen gui) {
        return Creed.popupAnimation.isToggled() && ((gui instanceof ModernClickGui && PopupAnimation.clickGui.isEnabled()) || (gui instanceof GuiInventory && PopupAnimation.inventory.isEnabled()) || (gui instanceof GuiChest && PopupAnimation.chests.isEnabled()));
    }
    ...
    public void onGuiOpen(final GuiOpenEvent event) {
        if (Creed.mc.currentScreen == null && PopupAnimation.lastGuiTimer.hasTimePassed(150L)) {
    ...
    public void assign()
    {
        Creed.popupAnimation = this;
    }

    public static void doScaling() {
        final float scaling = getScaling();
        final ScaledResolution res = new ScaledResolution(Creed.mc);
