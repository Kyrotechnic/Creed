package baby.creed.ui;

import baby.creed.ui.windows.ModuleWindow;
import baby.creed.ui.windows.Window;
import baby.creed.util.MilliTimer;
import baby.creed.util.StencilUtils;
import baby.creed.util.render.GLUtil;
import baby.creed.util.render.RenderUtils;
import baby.creed.Creed;
import baby.creed.managers.WindowManager;
...
public class ModernClickGui extends GuiScreen {
    ...
    public static baby.creed.ui.windows.Window selectedWindow;
    public static boolean settingsOpened;
    public static MilliTimer timer = new MilliTimer();
    private static double x;
    private static double y;
    private int settingsOffset = (int)getY()*2+9;
    ...
    public void initGui() {
        super.initGui();
        for (baby.creed.ui.windows.Window window : this.windowManager.windows) {
            window.initGui();
        }

        ScaledResolution sr = new ScaledResolution(Creed.mc);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtils.drawBorderedRoundedRect((float) getX() - 5, (float) getY() - 5, getWidth() + 10, getHeight() + 10, 6, 2 , Creed.themeManager.getPrimaryColor().getRGB(), Creed.themeManager.getSecondaryColor().getRGB());
        RenderUtils.drawBorderedRoundedRect((float)getX(), (float)getY(), 85.0f, getHeight(), 3.0f, 2.0f, Creed.themeManager.getPrimaryColor().getRGB(), Creed.themeManager.getSecondaryColor().getRGB());
        RenderUtils.drawBorderedRoundedRect((float)(getX() + 90.0), (float)getY(), getWidth() - 90.0f, 20.0f, 3.0f, 2.0f, Creed.themeManager.getPrimaryColor().getRGB(), Creed.themeManager.getSecondaryColor().getRGB());
        RenderUtils.drawBorderedRoundedRect((float)(getX() + 90.0), (float)(getY() + 25.0), getWidth() - 90.0f, getHeight() - 25.0f, 3.0f, 2.0f, Creed.themeManager.getPrimaryColor().getRGB(), Creed.themeManager.getSecondaryColor().getRGB());
        Fonts.getSecondary().drawCenteredString("Creed Client", (float)(getX() + 42.5), (float)(getY() + 6.0), Color.WHITE.getRGB());
        drawTopBar(mouseX, mouseY);
        for (baby.creed.ui.windows.Window window : this.windowManager.windows) {
            if(window.getName().equals("Settings")) {
                if (window == selectedWindow) {
                    RenderUtils.drawBorderedRoundedRect((float)(getX() + 5.0), (float)(getY() + (double)settingsOffset + 3.0), 75.0f, 12.0f, 4.0f, 2.0f, Creed.themeManager.getSecondaryColor().getRGB(), Creed.themeManager.getSecondaryColor().getRGB());
                }
                ...
                RenderUtils.drawBorderedRoundedRect((float)ModernClickGui.getX() + 88.0f, (float)ModernClickGui.getY() + 25.0f, ModernClickGui.getWidth() - 88.0f, ModernClickGui.getHeight() - 25.0f, 6.0f, 2.0f, Creed.themeManager.getPrimaryColor().getRGB(), Creed.themeManager.getPrimaryColor().getRGB());
                ...
                if (window == selectedWindow) {
                    RenderUtils.drawBorderedRoundedRect((float)(getX() + 5.0), (float)(getY() + (double)categoryOffset + 3.0), 75.0f, 12.0f, 4.0f, 2.0f, Creed.themeManager.getSecondaryColor().getRGB(), Creed.themeManager.getSecondaryColor().getRGB());
                }
    ...
    {
        Fonts.getPrimary().drawString("Welcome to Creed - v" + Creed.VERSION, (float)(getX() + 95), (float) (getY() + 6f), Color.WHITE.getRGB());
    ...
    public void onGuiClosed() {
        ...
        Creed.clickGui.setToggled(false);
    }
    @Override
