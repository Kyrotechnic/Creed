package baby.creed.ui.hud.impl;

import baby.creed.util.render.RenderUtils;
import baby.creed.Creed;
import baby.creed.modules.render.InventoryDisplay;
import baby.creed.ui.hud.DraggableComponent;
import baby.creed.ui.hud.HudVec;
...
public class InventoryHud extends DraggableComponent
{
    ...
    public InventoryHud() {
        ...
        this.setPosition(Creed.inventoryDisplay.x.getValue(), Creed.inventoryDisplay.y.getValue());
    }

    public HudVec drawScreen() {
        GL11.glPushMatrix();
        super.drawScreen();
        final ScaledResolution scaledResolution = new ScaledResolution(Creed.mc);
        int blur = 0;
        ...
        final String selected = Creed.inventoryDisplay.blurStrength.getSelected();
        ...
        for (int i = 9; i < 36; ++i) {
            ...
            final ItemStack stack = Creed.mc.thePlayer.inventory.getStackInSlot(i);
            if (stack != null) {
                Creed.mc.getRenderItem().renderItemAndEffectIntoGUI(stack, (int)x + 2 + 20 * (i % 9), (int)y);
        ...
        GL11.glPopMatrix();
        final InventoryDisplay display = Creed.inventoryDisplay;
    ...
    private void drawBorderedRoundedRect(final float x, final float y, final float width, final float height, final float radius, final float linewidth) {
        ...
        if (this.isHovered() && Creed.mc.currentScreen instanceof GuiChat) {
            RenderUtils.drawOutlinedRoundedRect(x, y, width, height, radius, linewidth, Color.white.getRGB());
        }
        else {
            RenderUtils.drawGradientOutlinedRoundedRect(x, y, width, height, radius, linewidth, Creed.themeManager.getSecondaryColor().getRGB(), Creed.themeManager.getSecondaryColor(3).getRGB(), Creed.themeManager.getSecondaryColor(6).getRGB(), Creed.themeManager.getSecondaryColor(9).getRGB());
        }
    }
